...
# Played once each time the lamp turns red (nogo). Defaults to the bundled long
# buzzer (buzz.wav, generated by make_buzz.py). Override with config.json
# {"error_sound": "..."} — an absolute path, or a bare /System/Library/Sounds
# name (e.g. "Basso.aiff"); set "" to silence.
DEFAULT_ERROR_SOUND = os.path.join(CODE_DIR, "buzz.wav")
...
class GreenlightApp(NSObject):
    def init(self):
        ...
        # "" silences the buzz; a bare name resolves under /System/Library/Sounds.
        snd = cfg.get("error_sound", DEFAULT_ERROR_SOUND)
        if snd and not os.path.isabs(snd):
            snd = f"/System/Library/Sounds/{snd}"
        self.error_sound = snd
        self.blink_on = True
        self.mtime = 0
        self.wix = None
    ...
    def play_error_sound(self):
        # Fire-and-forget: afplay blocks for the clip's length, so never wait on
        # it here or the poll timer stalls. Best-effort — a missing file is silent.
        if not self.error_sound or not os.path.exists(self.error_sound):
            return
        try:
            subprocess.Popen(
                ["afplay", self.error_sound],
